...
export default function CardDescription({title, text, className}){
    ...
    return (
            <div className={`${style.cardDescriptionCard} ${className}`}>
                <h3 className={`${style.cardDescriptionTitle} d-flex justify-between`} onClick={hand}>{title}<i className={ isHidden ? `${style.cardDescriptionIcon} fa-solid fa-chevron-down` : " fa-solid fa-chevron-up"} onClick={handleShowCardHidden}></i></h3>

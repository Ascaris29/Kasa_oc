...
export default function APropos(){
    ...
    const {classDisplay, setClassDisplay} = useState("aproposCardHidden");

    function handleShowCardHidden(){
        setClassDisplay("aproposCardShow");
    }
    return (
            <div className={`${style.apropos} p-30`}>
                ...
                <div className={`${style.aproposContent} d-flex flex-column`}>
                    ...
                    <div className={`${style.aproposCard} d-flex flex-column`}>
                        <div className={`${style.aproposCardTitle} d-flex justify-between`}>Fiabilité <i class="fa-solid fa-chevron-up"></i></div>
                        <div className={`${ aproposCardHidden ? "style.aproposCardHidden" : "style.aproposCardShow"}`}>Les annonces postées sur Kasa garantissent une fiabilité totale. Les photos sont conformes aux logements, et toutes les informations sont régulièrement vérifiées par nos équipes.</div>
                    </div>

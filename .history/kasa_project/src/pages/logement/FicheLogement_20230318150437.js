...
export default function FicheLogement(){
    ...
    function showEquipment(){
        return arrayEquipments.forEach(i => { return ( <p> {i} </p> ) } )
    }
        ...
        <>
            <div className={`${style.fichelogement} flex-fill d-flex flex-column`}>
                ...
                <div className={`${style.fichelogementBlocs} d-flex`}>
                        <CardDescription classNameBloc={`${style.fichelogementCard}`} title="Description" text={donnée.description}/>
                        <CardDescription classNameBloc={`${style.fichelogementCard}`} title="Équipements" text={showEquipment()} />
                </div>
            </div>
        </>

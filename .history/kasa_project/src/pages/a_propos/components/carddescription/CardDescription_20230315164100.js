...
export default function CardDescription({title, text, className}){
    ...
    return (
            <div className={`${style.cardDescriptionCard}`}>
                <h3 className={`${style.cardDescriptionTitle} d-flex justify-between ${class}`}>{title}<i className={`${style.cardDescriptionIcon} fa-solid fa-chevron-up`} onClick={handleShowCardHidden}></i></h3>

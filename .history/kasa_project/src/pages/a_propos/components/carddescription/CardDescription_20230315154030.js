...
export default function CardDescription(title, text, style){
    return (
        <div className={`${style.aproposContent} d-flex flex-column justify-between`}>
                    <div className={`${style`}>
                        <h3 className={`${style.aproposCardTitle} d-flex justify-between`}>Fiabilité <i className={`${style.aproposIcon} fa-solid fa-chevron-up`} onClick={handleShowCardHidden}></i></h3>
                        <p className={isHidden ? `${style}` : `${style.aproposCardShow}`}>Les annonces postées sur Kasa garantissent une fiabilité totale. Les photos sont conformes aux logements, et toutes les informations sont régulièrement vérifiées par nos équipes.</p>
                    </div>

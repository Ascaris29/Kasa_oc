...
export default function CardDescription({title, text, classNameCard, classNameTitle, classNameDescription}){
    ...
    return (
            <div className={`${style.cardDescriptionCard} ${classNameCard}`}>
                <h3 className={`${style.cardDescriptionTitle} ${cla}d-flex justify-between`} onClick={handleShowCardHidden}>{title}<i className={ isHidden ? `${style.cardDescriptionIcon} fa-solid fa-chevron-down` : " fa-solid fa-chevron-up"} onClick={handleShowCardHidden}></i></h3>
